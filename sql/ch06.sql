...
GROUP BY 지역;     -- 가장 큰 평균값 11025.5152
...
-- WHERE NOT EXISTS (SELECT * FROM 주문 WHERE 고객번호 = 고객.고객번호); 시
-- 한번도 주문한 적 없는 고객의 정보 출력
...
FROM 고객,
     (
     	...
     	FROM 고객
     	GROUP BY 도시
     ) AS 도시별요약    -- 반드시 별명 지정 필요
WHERE 고객.도시 = 도시별요약.도시;  -- 별명을 테이블명처럼 사용 가능
...
FROM 고객, (SELECT 고객번호
			...
			,MAX(주문일) AS 최종주문일
			FROM 주문
			GROUP BY 고객번호) AS 고객별주문
WHERE 고객.고객번호 = 고객별주문.고객번호;
...
SELECT SUM(주문수량) AS 주문수량합
FROM 주문세부, (SELECT * FROM 제품 WHERE 제품명 LIKE '%아이스크림') AS 아이스크림
WHERE 주문세부.제품번호 = 아이스크림.제품번호;

-- 4. '서울특별시' 고객들에 대한 주문년도별 주문건수를 보이시오.
SELECT YEAR(주문일) AS 주문년도, COUNT(*) AS 주문번호
FROM 주문, (SELECT 고객번호 FROM 고객 WHERE 도시 = '서울특별시') AS 서울고객
WHERE 주문.고객번호 = 서울고객.고객번호
GROUP BY 주문년도;