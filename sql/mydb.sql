...
-- 테이블 생성
--	PRIMARY KYE : 기본키로 지정(unique, not null 속성이 기본적으로 있음)
--  UNIQUE : 중복되지 않는 동일 컬럼값
-- NOT NULL : NULL이 되면 안 되는 값
...
-- auto increment 속성 컬럼은 0이면, 자동 증가함
INSERT INTO `member` VALUES (0, 'su', '1234', '수선화');

-- 레코드 수정하기
-- where 생략 시 모든 레코드가 수정되어버림.
