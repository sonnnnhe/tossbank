...
export function DynamicList() {
  ...
  const handleAddItem = () => {
    const newList = { id: list.length + 1, text: newItem };
...
export function Json() {
  ...
  useEffect(() => {
    fetch(url)
      .then((response) => {
        if (!response.ok) {
          throw Error("couldn't fetch the data that resource");
        }
  ...
  return (
    <div>
      <input
        ...
        value={newItem}
        onChange={(e) => setNewItem(e.target.value)}
        placeholder="항목 입력(id, User ID, Title, Body)"
      />
      <button onClick={handleAddItem}>추가하기</button>
      <ul>
        {lists.map((list) => (
...
// 프론트                                  백엔드
// html/css/js(MPA)  <통신>
// 리액트,Vue(SPA)    HTTP(Rest API)    Java Spring F/W (60%)
// APP(aOS, iOS)       Socket(Tcpip)      Node.js Express
//                          webSocket         Python Flask,FastAPI
