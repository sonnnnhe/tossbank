...
export function NumberList() {
  ...
  return (
    <>
      <p>Number List</p>
      <ul>
        {/* List.jsx:18 Warning: Each child in a list should have a aunique "key" prop. */}
        {/* key로 배열의 인덱스를 사용하면 안됨.
            배열의 데이터 순서가 바뀌거나 추가/삭제가 되면, 리액트 모든 항목을 다시 렌더링 해야함.
            입력의 포커스나 애니메이션효과 등의 상태를 유지할 수 없다.
         */}
        {nums.map((number, index) => (
          <li key={index}>{number}</li> // key를 넣어줘야 내부적으로 오류 미발생
        ))}
      </ul>
    </>
  );
