...
// Appp7.js import * as E from "7-hook/Hook"
...
//useCallback : 의존성배열이 바뀌면 콜백함수를 반환

// 2. useEffect 훅 : 사이드 이펙트 관리 훅
//    함수형 컴퍼넌트의 생명주기 함수를 지원한다.
//    1. 마운트 : useEffect( ()=>{}, [] );
//    2. 언마운트 : useEffct ( () => {return ()=>{}}, [] );
//    3. 업데이트(props, state가 업데이트될 때 호출됨) : useEffect(()=>{}, [상태변수(여러개 가능)]);
...
// 3. useMemo 훅
//    특정 연산의 결과를 memoization(저장)하여
//    불필요한 재계산을 방지하는데 사용된다.
//    컴퍼넌트가 렌더링 될때마다 매번 계산해야하는 대신
...
export function Counter4() {
  ...
  // number 상태변수가 변경되지 않는 한, double값은
  // 재연산되지 않음. (다른 변수의 상태 변화에는 double값이 바뀌지 않음 => useEffect의 update시는?)
  // 일반 함수로 구현했을 시 다른 상태 변수가 변했을 때도 연산이 됨. 즉 재연산이 발생하는 것.
  const double = useMemo(() => {
    console.log("두배 연산중...");
    return number * 2;
  }, [number]); // number가 변경됐을 때 콜백함수 실행됨

  // function Dodouble(number) {
  //   console.log("Dodouble...");
  //   return number * 2;
  // }
  // const double2 = Dodouble(number);
...
export function Counter6() {
  ...
  const handleClick = () => {
    setCount(count + 1); // 주석 처리 시 리렌더링 X, clickCountRef의 값은 계속 변함
    clickCountRef.current += 1; // 주석 처리 시 리렌더링 O
...
export function Counter7() {
  ...
  // 초기값 null. 밑의 ref={inputRef} 코드를 만나면
